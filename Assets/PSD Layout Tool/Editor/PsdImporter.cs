namespace PsdLayoutTool
{
   ...
   // using TMPro;
    ...
    public static class PsdImporter
    {
        ...
        {
            Debug.Log(layer.Name + "  "+layer.Name.ContainsIgnoreCase("Button"));
            ...
            else
            {
                ...
                if (LayoutInScene || CreatePrefab)
                {
                    GameObject preexiting = GameObject.Find(GetFullName(layer));

                    GameObject currentGroupGameObject = preexiting == null ? new GameObject(layer.Name) : preexiting;
        ...
        {
            GameObject preexisting = GameObject.Find(PsdName);
            Canvas = preexisting ?? new GameObject(PsdName);


            Canvas canvas = Canvas.GetComponent<Canvas>()!= null ? Canvas.GetComponent<Canvas>() : Canvas.AddComponent<Canvas>();
        ...
        private static Image CreateUIImage(Layer layer)
        {
            ...
            GameObject gameObject = preexiting == null ? new GameObject(layer.Name) : preexiting;
            gameObject.transform.position = new Vector3(x + (width / 2), y - (height / 2), currentDepth);
            gameObject.transform.parent = currentGroupGameObject.transform;
            ...
            //  DefyVisualElement defyVisualElement = gameObject.GetComponent<DefyVisualElement>() ?? gameObject.AddComponent<DefyVisualElement>();


            return image;
        }

        private static string GetFullName(Layer layer)
        {
            return currentGroupGameObject.name + "/" + layer.Name;
        }
        ...
        private static void CreateUIText(Layer layer)
        {
            ...
            GameObject gameObject = GameObject.Find(GetFullName(layer))?? new GameObject(layer.Name);
            gameObject.transform.position = new Vector3(x + (width / 2), y - (height / 2), currentDepth);
            gameObject.transform.parent = currentGroupGameObject.transform;
            ...
            //  TextMeshProUGUI textUI = gameObject.GetComponent<TextMeshProUGUI>() ?? gameObject.AddComponent<TextMeshProUGUI>();
            ...
            /// Localization
            /// 
            //Localize localize = gameObject.GetComponent<Localize>() ?? gameObject.AddComponent<Localize>();
            //localize.mTarget = gameObject.GetComponent<TextMeshProUGUI>();
            // localize.SetTerm(layer.Name);
        }
        ...
        private static void CreateUIButton(Layer layer)
        {
            ...
            Button button = image.gameObject.GetComponent<Button>() ?? image.gameObject.AddComponent<Button>();
            // DefyuButton defyuButton = image.gameObject.GetComponent<DefyuButton>() ?? image.gameObject.AddComponent<DefyuButton>();
